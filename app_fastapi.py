...
<head>
    ...
    <style>
        * {
        ...
        .project-card.current-skills-project {
            border-left: 5px solid #28a745;
            background: linear-gradient(to right, #f0fff4 0%, white 10%);
        }
        .project-card.missing-skills-project {
            border-left: 5px solid #667eea;
            background: linear-gradient(to right, #f0f4ff 0%, white 10%);
        }
                    ...
                    if (data.project_recommendations && Object.keys(data.project_recommendations).length > 0) {
                        ...
                        for (const [track, projects] of Object.entries(data.project_recommendations)) {
                            ...
                            projects.forEach((project, idx) => {
                                // Determine project type: first project uses current skills, second focuses on missing skills
                                const isCurrentSkills = idx === 0;
                                const projectType = isCurrentSkills ? 'current-skills' : 'missing-skills';
                                const badgeText = isCurrentSkills ? '✨ Build with Your Current Skills' : '🎯 Learn Missing Skills & Fill the Gap';
                                const badgeClass = isCurrentSkills ? 'project-current-skills' : 'project-missing-skills';
                                
                                html += `
                                    <div style="margin-bottom: 20px;">
                                        <span class="project-type-badge ${badgeClass}">${badgeText}</span>
                                    </div>
                                    <div class="project-card ${projectType}-project">
